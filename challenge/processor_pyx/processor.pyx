...
cdef void processCsv(const char* csv, const char* selectedColumns, const char* rowFilterDefinitions):
    cdef:
        str csv_data = csv.decode('utf-8')
        str columns = selectedColumns.decode('utf-8')
        str filters = rowFilterDefinitions.decode('utf-8')
    process_csv(csv_data, columns, filters)

cdef void processCsvFile(const char* csvFilePath, const char* selectedColumns, const char* rowFilterDefinitions):
    cdef:
        str csv_file_path = csvFilePath.decode('utf-8')
        str columns = selectedColumns.decode('utf-8')
        str filters = rowFilterDefinitions.decode('utf-8')
    process_csv_file(csv_file_path, columns, filters)
